...
// ---------- IMPORT (dari Excel, sudah di-parse jadi JSON di frontend) ----------
// body: { rows: [{ nama, site, wilayah, ip_address, latitude, longitude }, ...] }
app.post("/api/import", (req, res) => {
  const rows = Array.isArray(req.body.rows) ? req.body.rows : [];
  if (!rows.length) return res.status(400).json({ error: "Tidak ada data untuk diimpor." });

  // siapkan lookup site & area
  const sites = db.prepare("SELECT * FROM sites").all();
  const siteByName = {};
  sites.forEach(s => { siteByName[s.name.toLowerCase()] = s.id; });

  const getArea = db.prepare("SELECT id FROM areas WHERE site_id = ? AND lower(name) = lower(?)");
  const insArea = db.prepare("INSERT INTO areas (site_id, name) VALUES (?, ?)");
  const insAp = db.prepare(`INSERT INTO access_points (name, ip_address, latitude, longitude, area_id)
                            VALUES (?, ?, ?, ?, ?)`);
  ...
  rows.forEach((row, i) => {
    ...
    lat = (lat === "" || lat == null) ? null : parseFloat(lat);
    lng = (lng === "" || lng == null) ? null : parseFloat(lng);

    if (!nama || !siteName || !wilayah || !ip) {
      errors.push(`Baris ${line}: nama/site/wilayah/IP wajib diisi.`); return;
    }
    if (!isValidIp(ip)) { errors.push(`Baris ${line}: IP "${ip}" tidak valid.`); return; }
    const siteId = siteByName[siteName.toLowerCase()];
    if (!siteId) { errors.push(`Baris ${line}: site "${siteName}" tidak dikenal.`); return; }

    // cari / buat wilayah
    let area = getArea.get(siteId, wilayah);
    let areaId = area ? area.id : insArea.run(siteId, wilayah).lastInsertRowid;

    insAp.run(nama, ip, isNaN(lat) ? null : lat, isNaN(lng) ? null : lng, areaId);
    added++;
  });
